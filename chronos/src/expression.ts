...
import { Chronos, _getNow } from './chronos.js';
import type { Options } from './types.js';

declare module './chronos.js' {
    interface Chronos {
        parse(str: string, options?: Options): Date;
        parseRange(str: string, options?: Options): [Date, Date];
...
function _compile(t: Chronos): void {
    const ciKeys = (k: string) => k === k.toLowerCase();
...
function _ensure(t: Chronos): void {
    if (!t._re) _compile(t);
}

function _one(t: Chronos, str: string, options: Options): Date {
    ...
    let date: Date, anchor: string | null | undefined, rest: string;
...
Chronos.prototype.parse = function (this: Chronos, str: string, options: Options = {}): Date {
    _ensure(this);
    if (/\.\.| - | to |>/.test(str)) throw new Error('Use parseRange() for range expressions');
    return _one(this, str, options);
};

Chronos.prototype.parseRange = function (this: Chronos, str: string, options: Options = {}): [Date, Date] {
    _ensure(this);
...
Chronos.prototype.parseDuration = function (this: Chronos, str: string): number {
    _ensure(this);
